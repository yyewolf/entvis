...
type CardQuery struct {
	...
	withOwnedBy             *UserQuery
	withFavedUsers          *UserQuery
...
func (cq *CardQuery) QueryOwnedBy() *UserQuery {
...
// QueryFavedUsers chains the current query on the "faved_users" edge.
func (cq *CardQuery) QueryFavedUsers() *UserQuery {
	query := (&UserClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (cq *CardQuery) Clone() *CardQuery {
	...
	return &CardQuery{
		...
		withFavedUsers:          cq.withFavedUsers.Clone(),
...
func (cq *CardQuery) WithOwnedBy(opts ...func(*UserQuery)) *CardQuery {
	query := (&UserClient{config: cq.config}).Query()
...
// WithFavedUsers tells the query-builder to eager-load the nodes that are connected to
// the "faved_users" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CardQuery) WithFavedUsers(opts ...func(*UserQuery)) *CardQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withFavedUsers = query
	return cq
}
...
func (cq *CardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Card, error) {
	...
	if query := cq.withOwnedBy; query != nil {
		if err := cq.loadOwnedBy(ctx, query, nodes, nil,
			func(n *Card, e *User) { n.Edges.OwnedBy = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withFavedUsers; query != nil {
		if err := cq.loadFavedUsers(ctx, query, nodes,
			func(n *Card) { n.Edges.FavedUsers = []*User{} },
			func(n *Card, e *User) { n.Edges.FavedUsers = append(n.Edges.FavedUsers, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CardQuery) loadOwnedBy(ctx context.Context, query *UserQuery, nodes []*Card, init func(*Card), assign func(*Card, *User)) error {
...
func (cq *CardQuery) loadFavedUsers(ctx context.Context, query *UserQuery, nodes []*Card, init func(*Card), assign func(*Card, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(card.FavedUsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(card.FavedUsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(card.FavedUsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(card.FavedUsersPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "faved_users" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (cq *CardQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(card.Table, card.Columns, sqlgraph.NewFieldSpec(card.FieldID, field.TypeString))
	_spec.From = cq.sql
